...
void OnReceiveObjects(const std::string &buffer)
{
    ...
    std::time_t current_time_t = system_clock::to_time_t(current_time);
    std::cout << "Current time: " << std::ctime(&current_time_t) << std::endl;
    auto microsec = duration_cast<microseconds>(current_time.time_since_epoch()) % 1000000;
    std::cout << "Current time with microsecond precision: " << std::put_time(std::localtime(&current_time_t), "%F %T") << "." << std::setfill('0') << std::setw(6) << microsec.count() << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    ros::NodeHandle nh;
    radar_obj = nh.advertise<conti_radar::radar_obj>("radar_srr_obj", 10000);
